...
#include "mpif.h"
  ...
  real(dp) :: H,beta,B0,p0,amp,rhofloor,polytrop, pi,amps_den,amps_p,amps_ux,amps_uy,amps_uz,ampc_den,ampc_p,ampc_ux,ampc_uy,ampc_uz
  real(dp) :: rho,rhovx,rhovy,rhovz,Bxc,Byc,Bzc,E,csq,P,P_mid,nx_eigen,nz_eigen
  real :: rvalue
  !
  ! type :
  ...
  !
  namelist /init_params/d0,beta,gamma,polytrop,amp,type,floor,zfloor,dfloor,smooth,addmass,massDiff,stratif,rho_cool, c2_cool, t_cool,nx_eigen,nz_eigen,amps_den,amps_p,amps_ux,amps_uy,amps_uz,ampc_den,ampc_p,ampc_ux,ampc_uy,ampc_uz
  !
  ...
  amp=1.d-1
  type='entropy'
  floor=.false. ; smooth=.false. ; addmass=.false. ; massDiff=.false. ; stratif = .false.
  zfloor=5.
  ...
  !Set vertical profile
    do l=1,ngrid
     do k=ku1,ku2
           do j=ju1,ju2
              do i=iu1,iu2         
                 ! density :uin(l,i,j,k,1) =  d0+0.5455*COS(2*pi*i/(iu2-iu1-5))
  		 uin(l,i,j,k,1) = d0*(1.d0 + amp*amps_den*SIN(2.d0*pi*((nx_eigen*x(i)/(xmax-xmin))+(nz_eigen*z(k)/(zmax-zmin))))+amp*ampc_den*COS(2.d0*pi*((nx_eigen*x(i)/(xmax-xmin))+(nz_eigen*z(k)/(zmax-zmin)))))
                 ! add a uniform radial velocity to initiate an epicyclic oscillation :
                  uin(l,i,j,k,2) =  uin(l,i,j,k,1)*amp*amps_ux*SIN(2.d0*pi*((nx_eigen*x(i)/(xmax-xmin))+(nz_eigen*z(k)/(zmax-zmin))))+uin(l,i,j,k,1)*amp*ampc_ux*COS(2.d0*pi*((nx_eigen*x(i)/(xmax-xmin))+(nz_eigen*z(k)/(zmax-zmin))))

		 uin(l,i,j,k,3) = uin(l,i,j,k,1)*amp*amps_uy*SIN(2.d0*pi*((nx_eigen*x(i)/(xmax-xmin))+(nz_eigen*z(k)/(zmax-zmin)))) + uin(l,i,j,k,1)*amp*ampc_uy*COS(2.d0*pi*((nx_eigen*x(i)/(xmax-xmin))+(nz_eigen*z(k)/(zmax-zmin))))
		 uin(l,i,j,k,4) = uin(l,i,j,k,1)*amp*amps_uz*SIN(2.d0*pi*((nx_eigen*x(i)/(xmax-xmin))+(nz_eigen*z(k)/(zmax-zmin)))) + uin(l,i,j,k,1)*amp*ampc_uz*COS(2.d0*pi*((nx_eigen*x(i)/(xmax-xmin))+(nz_eigen*z(k)/(zmax-zmin))))!uin(l,i,j,k,1)*amp*0.0*COS(2.d0*pi*((nx_eigen*x(i)/(xmax-xmin))))
                 ! Vertical magnetic field
                 ! Energy in the case of an entropy wave (no magnetic field)


                    uin(l,i,j,k,5) = ((P_mid+amps_p*amp*SIN(2.d0*pi*((nx_eigen*x(i)/(xmax-xmin))+(nz_eigen*z(k)/(zmax-zmin))))+ampc_p*amp*COS(2.d0*pi*((nx_eigen*x(i)/(xmax-xmin))+(nz_eigen*z(k)/(zmax-zmin)))))/0.4)+ 0.5*((uin(l,i,j,k,2)**2)+((uin(l,i,j,k,3)-0.03*x(i))**2)+(uin(l,i,j,k,4)**2))/uin(l,i,j,k,1)
