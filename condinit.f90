module stratified
  ...
  logical :: floor,smooth,addmass,massDiff,stratif,resist,IsoRestart
...
#include "mpif.h"
...
#endif
  !
  namelist /init_params/d0,beta,gamma,amp,n,type,dirname,floor,zfloor,dfloor,smooth,addmass,massDiff,stratif,rho_cool, c2_cool, t_cool,kappa,resist,IsoRestart, cool_const
  !
  ...
  type='l'
  dirname=''
  floor=.false. ; smooth=.false. ; addmass=.false. ; massDiff=.false. ; stratif = .true.; resist=.false.; IsoRestart=.false.
  zfloor=3.
  ! parameters of the cooling function :
  rho_cool=1.d-2  ! density cutoff
  c2_cool=1.0     ! equilibrium temperature
  ...
  do l=1,ngrid
     do k=1,nz+1
        ...
        if ((type=='r').or.(type=='z').or.(type=='y').or.(type=='g')) then
	   ...
	   if (type=='g') then
		 do j=ju1,ju2
                     uin(l,iu1:iu2,j,k,8     ) = B0*sin(2.d0*pi*x(iu1:iu2))*exp(-abs(z(k))/1.4/H)
               !      uin(l,iu1:iu2,j,k,nvar+3) = B0*sin(2.d0*pi*x(iu1:iu2))*exp(-z(k)/1.4/H)
                     uin(l,iu1:iu2,j,k,6     ) = -B0*cos(2.d0*pi*x(iu1:iu2))*exp(-abs(z(k))/1.4/H)/(2.d0*pi)/H/gamma
              !       uin(l,iu1:iu2,j,k,nvar+3) = -B0*cos(2.d0*pi*x(iu1:iu2))*exp(-z(k)/1.4/H)/(2.d0*pi)/H
                 end do
           end if
