...
#include "mpif.h"
 ...
 ! WRITE(*,*) 'H :', H, '   P_mid :', P_mid
  ...
  !CALL C2DICHOTOMY
   IF (rho_cool .GT. 0.d0) THEN
      c2_cool = 1.d0 - t_cool*nu*(1.5d0*Omega0)**2*exp(d0/rho_cool)
   ELSE
      c2_cool = 1.d0 - t_cool*nu*(1.5d0*Omega0)**2
   END IF
  ...
  do l=1,ngrid
     do k=ku1,ku2
        ...
        ! background density :
        uin(l,:,:,k,1)=y0(2) !d0*exp(-z(k)**2/(2.d0*H**2))
...
SUBROUTINE DERIVE0(z0,y0,dzy0)
  ...
  IF (rho_cool .GT. 0.d0) THEN
     dzy0(2) = dzy0(1)/(c2/gamma + rho/rho_cool*nu/gamma*(1.5d0*Omega0)**2*t_cool*exp(rho/rho_cool) )
  ELSE
     dzy0(2) = dzy0(1)/(c2/gamma)
  END IF
  dzy0(3) = 0.d0 !rho*nu*(1.5d0*Omega0)**2 + L_cool
