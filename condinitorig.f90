module stratified
  ...
  DOUBLE PRECISION :: rho_cool, c2_cool, t_cool,nu_init,kappa_init
...
#include "mpif.h"
  ...
  real(dp) :: H,beta,B0,p0,amp,rhofloor,polytrop, pi, nu_temp, kappa_temp
  ...
  !
  namelist /init_params/d0,beta,gamma,polytrop,amp,type,floor,zfloor,dfloor,smooth,addmass,massDiff,stratif,rho_cool, c2_cool, t_cool,nu_init,kappa_init
  !
  ...
  H=1.d0/(sqrt(gamma)*Omega0)
  ! magnetic field strength such that beta is that of the input file at the midplane :
  B0=sqrt(2.d0/gamma*d0*1.d0**2/beta)
 ...
 ! WRITE(*,*) 'H :', H, '   P_mid :', P_mid
  ...
  ! temporarily set nu to compute the stationary state :
  nu_temp = nu
  kappa_temp = kappa
  nu = nu_init
  kappa = kappa_init
...
SUBROUTINE c2DICHOTOMY
  ...
  DO WHILE((c2_coolmin .LT. 0.d0).AND.(niteration .LT. 100))
     ...
     END IF
  END DO 
  IF (niteration .GE. 100) WRITE(*,*) 'Dichotomy has not converged !!'

  niteration = 0
  ! Find c2_cool by dichotomy :
  DO WHILE(((c2_coolmax-c2_coolmin)/c2_coolmin .GT. 1.d-12).AND.(niteration .LT. 100))
     niteration = niteration + 1
